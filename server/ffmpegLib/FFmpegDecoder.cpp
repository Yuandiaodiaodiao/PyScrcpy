...
FFmpegDecoder::FFmpegDecoder(SocketConnection *connection, FrameCache *cache,
                             SDL_Screen *screen)
    : connection(connection), cache(cache), screen(screen) {}
...
SDL_bool FFmpegDecoder::init() {
  ...
  avio_ctx = avio_alloc_context(buffer, BUF_SIZE, 0, this, read_socket_buffer,
                                NULL, NULL);
